...
def save_pic_titles(titles, file_path):
    saved_titles = set()  # 用于保存已保存的标题，确保唯一性
...
if __name__ == '__main__':
    ...
    # 保存标题到文件（确保每个标题只保存一次）
    titles_file_path = os.path.join(main_folder, 'titles.txt')
    save_pic_titles(pic_titles, titles_file_path)

    print(f"完成！共保存 {len(pic_titles)} 个图片页面标题。")
